...
public class ThreadParserService extends BaseChanService implements ChanIdentifiedService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (DEBUG)
            Log.i(TAG, "Handling board=" + boardCode + " threadNo=" + threadNo + " priority=" + priority);
        ...
        try {
            ...
            if (previousPostNum > 0 && thread.posts.length == 0) {
                ...
                if (!boardCode.equals(thread.board)) {
                    if (DEBUG)
                        Log.i(TAG, "Found inconsistent thread boardCode=" + thread.board + ", repairing to=" + boardCode);
                    thread.board = boardCode;
                }
                if (DEBUG)
